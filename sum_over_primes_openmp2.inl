...
#define FMPIMPL2_NAME(FN) CONCAT(FN,_openmp_impl2)
#define FMPTHREAD2_P2_NAME(FN) CONCAT(FN,_openmp_thread2_p2)
#define FMPTHREAD2_PHIS3_NAME(FN) CONCAT(FN,_openmp_thread2_phi_s3)
#define FMPTHREAD2_PHIS2_NAME(FN) CONCAT(FN,_openmp_thread2_phi_s2)

/*                                                                              */
/* Sum over Primes OpenMP implementation                                        */
/*   P2, phi S3, phi S2 are independently threaded                              */
/*                                                                              */

//
// thread function for phi S3
//

static inline T FMPTHREAD2_PHIS3_NAME(NAME) (
    u64 X, u64 Y, uint K, uint A, uint A_x4, u64 l, uint B, uint prime_count, const u64 prime[static prime_count+1],
    const uint phi_lpf[static Y+1], const i8 phi_mu[static Y+1], bool block_sieve[static B], T block_tree[static B+1],
    T out_phi_last_block[static A_x4+1], T out_phi_S3_block[static A_x4+1]
) {
...
static inline T FMPTHREAD2_PHIS2_NAME(NAME) (
    u64 X, u64 x_2, u64 Y, uint K, uint A, uint A_x3, uint A_x_div_y_2, uint A_x4, u64 l, int B,
    uint prime_count, const u64 prime[static prime_count+1],
    T Fn_holder[static B+1], bool block_sieve[static B],
    T* out_Fn_block, T* out_phi_S2_block
) {
    if(l > x_2) {
        *out_phi_S2_block = *out_Fn_block = TCONV_64(0);
        return TCONV_64(0);
    }

    T phi_S2 = TCONV_64(0), phi_S2_accum = TCONV_64(0);
    const u64 h = l + B;

    // fully sieve block
    memset(block_sieve, 0x00, sizeof(bool[B]));
    mark_composite_with_primes(l,B,block_sieve, 1,A+1, prime);
...
//
// thread function for P2
//

static inline T FMPTHREAD2_P2_NAME(NAME) (
    u64 X, u64 x_2, u64 Y, uint K, uint A, u64 l, int B,
    uint prime_count, const u64 prime[static prime_count+1],
    T Fn_holder[static B+1], bool block_sieve[static B], bool q_sieve[static B],
    T* out_Fn_block, T* out_P2_FI_block
) {
    if(l*Y > X) {
        *out_Fn_block = *out_P2_FI_block = TCONV_64(0);
        return TCONV_64(0);
    }

    T P2 = TCONV_64(0), P2_Fi_accum = TCONV_64(0);
    const u64 h = l + B;
...
static inline T FMPIMPL2_NAME(NAME) (u64 X, uint TC, u64 Y, u64 B, uint K) {
    //
    ...
    // will be used for everything below
    ...
    for(uint bi = 0 ; ; bi += TC) {
        ...
        #pragma omp parallel for num_threads(TC) reduction(+: phi_S3)
        for(uint t = 0 ; t < TC ; ++t) {
            const T block_phi_S3 = FMPTHREAD2_PHIS3_NAME(NAME) (
                X,Y,K, A,A_x4, l+t*B,B, prime_count,prime, phi_lpf,phi_mu,
                (*block_sieve)[t], (*block_tree)[t], (*phi_last_block)[t], (*phi_S3_block)[t]
            );
            phi_S3 = TADD(phi_S3, block_phi_S3);
        }

        // finalize per-block phi S3 values
        for(uint t = 0 ; t < TC ; ++t) {
    ...
    free(phi_S3_block); free(phi_last_block); free(phi_last); free(block_tree);
    free(phi_mu); free(phi_lpf);
    ...
    for(uint bi = 0 ; ; bi += TC) {
        ...
        #pragma omp parallel for num_threads(TC) reduction(+: phi_S2)
        for(uint t = 0 ; t < TC ; ++t) {
            const T block_phi_S2 = FMPTHREAD2_PHIS2_NAME(NAME) (
                X,x_2,Y,K, A,A_x3,A_x_div_y_2,A_x4, l+t*B,B,
                prime_count,prime,
                (*Fn_holder)[t], (*block_sieve)[t], &Fn_accum_block[t], &phi_S2_block[t]
            );
            phi_S2 = TADD(phi_S2, block_phi_S2);
        }

        // finalize per-block phi values and update accumulators
        for(uint t = 0 ; t < TC ; ++t) {
    ...
    Fn_accum = TCONV_64(0);
    for(uint bi = 0 ; ; bi += TC) {
        const u64 l = Y+1 + bi*B;
        if(l*Y > X) break;

        #pragma omp parallel for num_threads(TC) reduction(+: P2,phi_S2,phi_S3)
        for(uint t = 0 ; t < TC ; ++t) {
            const T block_P2 = FMPTHREAD2_P2_NAME(NAME) (
                X,x_2,Y,K,A, l+t*B,B,
                prime_count,prime,
                (*Fn_holder)[t], (*block_sieve)[t], (*q_sieve)[t],
                &Fn_accum_block[t], &p2_FI_block[t]
            );
            P2 = TADD(P2, block_P2);
        }

        // finalize per-block P2 values and update accumulators
        for(uint t = 0 ; t < TC ; ++t) {
    ...
    free(q_sieve); free(Fn_holder); free(block_sieve); free(prime);
...
#undef FMPIMPL2_NAME
#undef FMPTHREAD2_P2_NAME
#undef FMPTHREAD2_PHIS3_NAME
#undef FMPTHREAD2_PHIS2_NAME
